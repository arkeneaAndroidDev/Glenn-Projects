...
public class SampleIapManager {
    ...
    private boolean level2ProductAvailableSecond;
    private boolean level2ProductAvailableThird;
    ...
    PurchaseStatus listner;

    public SampleIapManager(final Activity mainActivity,PurchaseStatus listner) {
    ...
    public void setAmazonUserId(final String newAmazonUserId, final String newAmazonMarketplace) {
        ...
        if (newAmazonUserId == null) {
            ...
            if (userIapData != null) {
                ...
                refreshLevel3Availability();
                refreshLevel4Availability();
            }
    ...
    public void enablePurchaseForSkus(final Map<String, Product> productData) {
        ...
        if(productData.containsKey(MySku.LEVEL3.getSku())){
            level2ProductAvailableSecond = true;
        }
        if(productData.containsKey(MySku.LEVEL4.getSku())){
            level2ProductAvailableThird = true;
        }
    }
    ...
    public void disablePurchaseForSkus(final Set<String> unavailableSkus) {
        if (unavailableSkus.contains(MySku.LEVEL2.toString())) {
            ...
            Toast.makeText(mainActivity, "the level2 product isn't available now! ", Toast.LENGTH_LONG).show();
        }
        if (unavailableSkus.contains(MySku.LEVEL3.toString())) {
            level2ProductAvailableSecond = false;
            Toast.makeText(mainActivity, "the level2 product isn't available now! ", Toast.LENGTH_LONG).show();
        }
        if (unavailableSkus.contains(MySku.LEVEL4.toString())) {
            level2ProductAvailableThird = false;
            Toast.makeText(mainActivity, "the level2 product isn't available now! ", Toast.LENGTH_LONG).show();
        }
    }
    ...
    public void purchaseFailed(final String sku) {
        Toast.makeText(mainActivity, "Purchase failed!", Toast.LENGTH_LONG).show();
    }
    ...
    public boolean isLevel2ProductAvailableSecond() {
        return level2ProductAvailableSecond;
    }

    public void setLevel2ProductAvailableSecond(final boolean level2ProductAvailable) {
        this.level2ProductAvailableSecond = level2ProductAvailable;
    }

    public boolean isLevel2ProductAvailableThird() {
        return level2ProductAvailableThird;
    }

    public void setLevel2ProductAvailableThird(final boolean level2ProductAvailable) {
        this.level2ProductAvailableThird = level2ProductAvailable;
    }
    ...
    public void disableAllPurchases() {
        ...
        this.setLevel2ProductAvailableSecond(false);
        this.setLevel2ProductAvailableThird(false);
        refreshLevel2Availability();
        refreshLevel3Availability();
        refreshLevel4Availability();
    }
    ...
    public void refreshLevel2Availability() {
        ...
        if(listner != null){
            listner.onPurchaseState(isLevel2ProductAvailable(), isLevel2ProductAvailableSecond(), isLevel2ProductAvailableThird(), 
    ...
    public void refreshLevel3Availability(){
        if (userIapData != null) {
            final EntitlementRecord entitlementRecord = dataSource.getLatestEntitlementRecordBySku(userIapData
                .getAmazonUserId(), MySku.LEVEL3.getSku());
            // Make sure the entitlement purchase record is not expired or
            // canceled
            if(entitlementRecord != null)
            level3Purchased = (EntitlementRecord.DATE_NOT_SET == entitlementRecord.cancelDate);
            if(listner != null){
                listner.onPurchaseState(isLevel2ProductAvailable(), isLevel2ProductAvailableSecond(), isLevel2ProductAvailableThird(), 
                        level2Purchased, level3Purchased, level4Purchased);
            }
    ...
    public void refreshLevel4Availability(){
        if (userIapData != null) {
            final EntitlementRecord entitlementRecord = dataSource.getLatestEntitlementRecordBySku(userIapData
                .getAmazonUserId(), MySku.LEVEL4.getSku());
            // Make sure the entitlement purchase record is not expired or
            // canceled
            if(entitlementRecord != null)
            level4Purchased = (EntitlementRecord.DATE_NOT_SET == entitlementRecord.cancelDate);
            if(listner != null){
                listner.onPurchaseState(isLevel2ProductAvailable(), isLevel2ProductAvailableSecond(), isLevel2ProductAvailableThird(), 
                        level2Purchased, level3Purchased, level4Purchased);
            }
    ...
    private void grantEntitlementPurchase(final Receipt receipt, final UserData userData) {
        ...
        Log.e("getSku", receipt.getSku()+"........"+userIapData.getAmazonMarketplace());
        Log.e("MYSKU", ".."+mySku);
        if ((mySku != MySku.LEVEL2)&&(mySku != MySku.LEVEL3)&&(mySku != MySku.LEVEL4)) {
        ...
        try {
            if (receipt.isCanceled()) {
                ...
                if (!verifyReceiptFromYourService(receipt.getReceiptId(), userData)) {
                    ...
                    Toast.makeText(mainActivity, "Purchase cannot be verified, please retry later.", Toast.LENGTH_LONG).show();
                    return;
                }
            ...
            Toast.makeText(mainActivity, "Purchase cannot be completed, please retry", Toast.LENGTH_LONG).show();
        }
