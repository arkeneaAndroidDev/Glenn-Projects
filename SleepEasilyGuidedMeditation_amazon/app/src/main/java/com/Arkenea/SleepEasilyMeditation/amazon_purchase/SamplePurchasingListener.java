...
public class SamplePurchasingListener implements PurchasingListener {
    ...
    public SamplePurchasingListener(final SampleIapManager iapManager, PurchaseStatus listner) {
    ...
    public void onUserDataResponse(final UserDataResponse response) {
        Log.e(TAG, "onGetUserDataResponse: requestId (" + response.getRequestId()
                   + ") userIdRequestStatus: "
    ...
    public void onProductDataResponse(final ProductDataResponse response) {
        ...
        Log.e(TAG, "onProductDataResponse: RequestStatus (" + status + ")");

        switch (status) {
        case SUCCESSFUL:
            Log.e(TAG, "onProductDataResponse: successful.  The item data map in this response includes the valid SKUs");
            final Set<String> unavailableSkus = response.getUnavailableSkus();
            Log.e(TAG, "onProductDataResponse: " + unavailableSkus.size() + " unavailable skus");
            iapManager.enablePurchaseForSkus(response.getProductData());
            Log.e("Product DAta", "......."+response.getProductData().toString()+";"+response.getUnavailableSkus());
            iapManager.disablePurchaseForSkus(response.getUnavailableSkus());
            iapManager.refreshLevel2Availability();
            iapManager.refreshLevel3Availability();
            iapManager.refreshLevel4Availability();
    ...
    public void onPurchaseUpdatesResponse(final PurchaseUpdatesResponse response) {
        Log.e(TAG, "onPurchaseUpdatesResponse: requestId (" + response.getRequestId()
    ...
    public void onPurchaseResponse(final PurchaseResponse response) {
        ...
        Log.e(TAG, "onPurchaseResponse: requestId (" + requestId
                   + ") userId ("
                   + userId
        ...
        case SUCCESSFUL:
            ...
            Log.e("receipt", "...."+(response.getRequestId().toString()+";"+receipt+";"+response.getUserData()));
            iapManager.handleReceipt(response.getRequestId().toString(), receipt, response.getUserData());
            iapManager.refreshLevel2Availability();
            iapManager.refreshLevel3Availability();
            iapManager.refreshLevel4Availability();
            ...
            if(listner != null)
                listner.onPurchaseSuccess(receipt.getSku());
            break;
